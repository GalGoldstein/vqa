...
class VQA(nn.Module):
    def __init__(self, gru_params: dict, label2ans_path: str, target_type: str, img_feature_dim: int, padding: int,
        ...
        self.linear_inside_relu_attention = nn.Linear(img_feature_dim + gru_params['question_hidden_dim'],
                                                      self.hidden_dim)
        # linear layer after relu activation before attention
        self.linear_after_relu_attention = nn.Linear(self.hidden_dim, 1, bias=False)

        # relu activation hidden representation of image
        self.linear_inside_relu_image = nn.Linear(img_feature_dim, self.hidden_dim)

        # relu activation hidden representation of question
        self.linear_inside_relu_question = nn.Linear(gru_params['question_hidden_dim'], self.hidden_dim)

        # relu activation last
        self.linear_inside_relu_last = nn.Linear(self.hidden_dim, self.hidden_dim)

        # last linear fully connected
        self.fc = nn.Linear(self.hidden_dim, self.num_classes, bias=False)
